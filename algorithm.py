...
docs = pd.read_csv('datasets/datasetALL.csv')
...
def get_music(x:str, return_id=True) -> list:
    ...
    all_idx = torch.topk(cosine_scores.flatten(), 50).indices
    cluster = []
    for i in all_idx:
       cluster.append([df.loc[int(i), "song"],df.loc[int(i), "artist"],df.loc[int(i), "text"]])
       print("name :", df.loc[int(i), "song"], df.loc[int(i),"artist"])
    return cluster